...
- **End boss stops the clock** — killing an instance's final boss (Lady Vashj, Kael'thas, Prince Malchezaar, …) freezes the timer immediately instead of only after leaving the instance. A "complete" line with the final time is printed, the panel shows a green "done" tag, and an optional boss killed afterwards in the same zone (e.g. Netherspite after Prince) extends the frozen time to that kill.
