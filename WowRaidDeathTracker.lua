...
border:SetBackdropBorderColor(0.22, 0.22, 0.28, 1)
...
-- Icon (Totenkopf aus Raid-Frame)
local icon = display:CreateTexture(nil, "OVERLAY")
icon:SetSize(14, 14)
icon:SetPoint("TOPLEFT", 7, -5)
icon:SetTexture("Interface\\RAIDFRAME\\ReadyCheck-Dead")
...
local resetBtnBorder = CreateFrame("Frame", nil, resetBtn, "BackdropTemplate")
resetBtnBorder:SetAllPoints()
resetBtnBorder:SetBackdrop({ edgeFile = "Interface\\Buttons\\WHITE8X8", edgeSize = 1 })
resetBtnBorder:SetBackdropBorderColor(0.38, 0.12, 0.12, 0.9)
...
-- Kreisförmiger Rahmen (Standard Minimap-Border)
local minimapBtnRing = minimapBtn:CreateTexture(nil, "BACKGROUND")
minimapBtnRing:SetTexture("Interface\\Minimap\\MiniMap-TrackingBorder")
minimapBtnRing:SetSize(40, 40)
minimapBtnRing:SetPoint("CENTER")

-- Dunkler Hintergrundkreis
local minimapBtnBg = minimapBtn:CreateTexture(nil, "ARTWORK")
minimapBtnBg:SetTexture("Interface\\Minimap\\UI-Minimap-Background")
minimapBtnBg:SetSize(20, 20)
minimapBtnBg:SetPoint("CENTER")
minimapBtnBg:SetVertexColor(0.1, 0.1, 0.15)

-- Skull-Icon
local minimapBtnIcon = minimapBtn:CreateTexture(nil, "OVERLAY")
minimapBtnIcon:SetTexture("Interface\\RAIDFRAME\\ReadyCheck-Dead")
minimapBtnIcon:SetSize(14, 14)
...
-- ----------------------------------------------------------------
local TALLY_MARKS = { "|", "||", "|||", "||||" }
local function GetTally(n)
    local s    = ""
    local full = math.floor(n / 5)
    local rest = n % 5
    for _ = 1, full do s = s .. "卌 " end
    if rest > 0 then s = s .. TALLY_MARKS[rest] end
    return s
end
...
function RaidDeathTrackerFrame:UpdateDisplay()
    ...
    for i = 1, math.min(TOP_N, #sorted) do
        local e   = sorted[i]
        local col = RANK_COLORS[i] or "|cffcccccc"
        local tal = GetTally(e.count)
        table.insert(lines, string.format(
            "%s#%d|r |cffdd4444%s|r  |cff666677%s|r |cff444455(%d)|r",
            col, i, e.name, tal, e.count
        ))
    end
    ...
    if extra > 0 then
        footer = string.format(
            "\n|cff33334a... +%d weitere|r  |cff444455Gesamt: %d|r",
            extra, total
        )
...
local function ActivateTestMode()
    ...
    display:Show()
    testBadge:Show()
    print("|cff00ff00[RDT]|r |cffff9900Testmodus:|r Dummy-Daten geladen. /rdt test clear zum Beenden.")
end
